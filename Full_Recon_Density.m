...
%% Multi-Window Phase Space Reconstruction with Full-Signal Density Plot
...
while true
    ...
    % Define windows across full signal length
    total_windows = floor(length(filtered_signal_data) / range_samples);
    full_time_labels = [];

    for i = 1:total_windows
        start_idx = (i-1) * range_samples + 1;
        end_idx = start_idx + range_samples - 1;

        % Ensure valid window
        if end_idx > length(filtered_signal_data)
            continue;
        end
        ...
        % Store phase space points
        phase_space_points = [X1; X2; X3];

        % Append phase space points to full dataset
        ...
        % Store window time
        full_time_labels(end+1) = (start_idx/sample_rate) + retain_start_time;
    end

    % Plot all density methods over full signal data
    figure;
    hold on;
    plot(full_time_labels, density_bounding_box, '-o', 'DisplayName', 'Bounding Box Density');
    plot(full_time_labels, density_convex_hull, '-o', 'DisplayName', 'Convex Hull Density');
    plot(full_time_labels, density_kde, '-o', 'DisplayName', 'KDE Density');
    xlabel('Time (s)');
    ylabel('Point Density');
    title('Point Density Over Full Signal Data');
    legend;
    grid on;
end
